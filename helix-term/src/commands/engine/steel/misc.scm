...
(provide cursor-lsp-location)
;;@doc
;; Get the LSP location of the cursor's current position under the given offset encoding.
(define cursor-lsp-location helix.cursor-lsp-location)

(provide get-lsp-initialize-result)
;;@doc
;;Get the language server's initialize result.
(define get-lsp-initialize-result helix.get-lsp-initialize-result)
